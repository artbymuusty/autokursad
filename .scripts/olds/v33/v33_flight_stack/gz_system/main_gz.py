...
from gz_system.gz_payload_actuator import (
    GOREV3_PICKUP_TARGET_COLOR, PAYLOAD_MODEL, PAYLOAD_MODEL_BY_SHAPE,
    VEHICLE_MODEL_NAME, GzPayloadActuator,
)
from gz_system.gz_pose_monitor import GzPoseMonitor
from gz_system.gz_hook_client import GzHookClient
...
# NOT __name__: this module is executed as a script (run_mission_v33_gz.sh runs
...
async def _run(config: dict, mission_id: str) -> None:
    ...
    # artik kurulmaz -- izleme, run_mission_v33_gz.sh'in otomatik baslattigi
    ...
    try:
        ...
        # PHASE 5.5 ADIM C -- /hook/state aboneliginin bootstrap'i.
        #
        # NEDEN TAM BURADA: bir ustteki pose_monitor.start() ile ayni gerekce
        # (F2 notu) -- gz-transport discovery (~2 s) takeoff'tan ONCE, kurulum
        # sirasinda odensin. Hook tarafinda bu bir optimizasyon degil DOGRULUK
        # meselesi: /hook/state latch'siz ve gecis basina TEK KEZ yayinlanir
        # (HookAttachSystem.cc:64, :127) ve plugin attach'i 2.485 ms'de
        # tamamlar (2026-08-23 canli olcum). Abonelik attach'ten SONRA
        # acilirsa gozlenecek tek mesaj yapisal olarak kacirilir -- joint
        # fiziksel olarak olussa bile timeout alinir. Legacy
        # gz_payload_actuator.py::_await_attach tam bu sirayla calisiyor
        # (once publish, sonra monitor); bu satir yeni yolu o hatadan bagimsiz
        # kurar.
        #
        # pose_monitor ENJEKTE EDILIYOR: client kendi GzPoseMonitor'unu
        # kurabilirdi ama o zaman /world/<world>/dynamic_pose/info'ya IKINCI
        # bir abonelik acilirdi. Tek cache paylasiliyor.
        #
        # Model adlari gz_payload_actuator.py'deki mevcut sabitlerden geliyor
        # (ikinci bir "payload_red" kopyasi olusmasin diye); o dosya bu adin
        # dayandigi PayloadInterlock degismezini de belgeliyor.
        #
        # HENUZ MISSION'A BAGLI DEGIL (kasitli): bu adim yalnizca
        # composition-root'tur. GazeboPayloadBackend/PayloadManager'in bu
        # client'i tuketmesi ayri bir wiring fazidir; MissionManager'in
        # davranisi bu satirlarla DEGISMEZ.
        hook_client = await GzHookClient.create(
            payload_model_name=PAYLOAD_MODEL % GOREV3_PICKUP_TARGET_COLOR,
            vehicle_model_name=VEHICLE_MODEL_NAME,
            pose_monitor=pose_monitor)
        logger.info("GzHookClient bootstrap tamam: hook_state_stream_ready=%s "
                    "(payload/ yolu icin hazir, henuz mission'a bagli degil).",
                    hook_client.is_state_stream_ready())

        # PHASE 6 -- payload/ yolunun gercek wiring'i.
        #
        # Backend client'i, PayloadManager backend'i ENJEKTE alir; ucu de
        # birbirinin somut tipini bilmez (payload/ paketi gz_system'i import
        # ETMEZ -- bagimlilik yalnizca bu composition root'ta kurulur).
        #
        # MISSION'A BAGLI DEGIL (kasitli, Phase 6 sinirlari): master FSM ve
        # Gorev3 fazlari hala GzPayloadActuator uzerinden calisir. Buradaki
        # payload_manager yalnizca ORNEKLENIR ve erisilebilir olur; onu
        # gorev akisina baglamak ayri bir MissionManager wiring fazidir.
        # Bu yuzden asagidaki iki satir mevcut gorev davranisini DEGISTIRMEZ.
        payload_backend = GazeboPayloadBackend(
            hook_client,
            payload_model_name=PAYLOAD_MODEL % GOREV3_PICKUP_TARGET_COLOR,
            vehicle_model_name=VEHICLE_MODEL_NAME,
            # {sekil: model} haritasi BURADA kurulur (gz_system, izinli);
            # payload/ paketi eslemeyi bilmez, yalnizca sozlugu alir.
            payload_models_by_shape=PAYLOAD_MODEL_BY_SHAPE)
        payload_manager = PayloadManager(payload_backend)
        logger.info("PayloadManager hazir (backend=GazeboPayloadBackend, state=%s) "
                    "-- Gorev 3 fazlarina PHASE 6.5'te baglandi.",
                    payload_manager.get_state().value)
        ...
        # Mission Flow V3 (F2): PayloadInterlock'u sarar, aynı publisher'ı
        # paylaşır -- completed_count/mission_order/GOREV_V3_SUCCESS event'leri
        # bu mission'ın kendi JSONL log'una (EventStore) otomatik akar.
        # PayloadInterlock'un sıra kuralına (Mavi Altıgen -> Kırmızı Üçgen)
        # dokunmaz, sadece üzerine gözlemlenebilir isimlendirme ekler.
        mission_v3_state = MissionV3State(interlock, publisher=publisher)
        ...
        sequencer = PayloadMissionSequencer(flight, centering, interlock, position_store, release_service,
                                             publisher=publisher, mission_v3_state=mission_v3_state)
        ...
        # PHASE 6.5: Gorev3 fazlari artik IPayloadActuator degil
        # payload/PayloadManager kullaniyor (supersede karari fiilen
        # uygulandi). `actuator` bu root'ta HALA duruyor cunku Gorev 2'nin
        # iki birakma yolu (release_service/gorev2) hala IPayloadActuator
        # uzerinden calisiyor -- supersede bu fazda KISMIdir.
        # DINAMIK HEDEF (2026-08-24): mission_v3_state gecirilir ki Gorev 3
        # 1st_mission/2nd_mission'i GERCEK tespit sirasindan okusun.
        pickup_phase = Gorev3PickupPhase(flight, camera, feed_detector, payload_manager, position_store,
                                          RectangleAlignmentStrategy(), centering,
                                          mission_v3_state=mission_v3_state)
        transport_phase = Gorev3TransportPhase(flight, position_store, centering,
                                                payload_manager)
        redrop_phase = Gorev3RedropPhase(flight, payload_manager, position_store,
                                          centering, mission_v3_state=mission_v3_state)
        ...
        try:
            ...
            await vision.stop()
            # ADIM C: pose_monitor'dan ONCE -- client onu ENJEKTE aldi,
            # sahibi degil; kendi /hook/state surecini kapatir, paylasilan
            # poz cache'ine DOKUNMAZ.
            await hook_client.stop()
