...
from payload.backends.real_payload_backend import (
    RealPayloadBackend, warn_if_uncalibrated,
)
...
async def _run(real_config: dict, gz_config: dict, mission_id: str) -> None:
    ...
    try:
        ...
        # PHASE 6.5: Gorev 3'un payload yolu icin DualPayloadActuator'in
        # payload/ karsiligi. Yeni bir secim mekanizmasi ICAT EDILMEDI --
        # repoda zaten secim yok, ayrim entry point duzeyinde yapiliyor;
        # bu blok bir ust satirdaki fan-out desenini birebir yansitir
        # (ikisini birden sur, uyumsuzlukta uyar, Real'i otoriter dondur).
        #
        # Gazebo tarafi kendi /hook/state bootstrap'ini ister (main_gz.py
        # ile ayni gerekce: abonelik attach'ten ONCE acilmali, yoksa
        # latch'siz tek-seferlik onay kacirilir).
        gz_pose_monitor = GzPoseMonitor()
        await gz_pose_monitor.start()
        gz_hook_client = await GzHookClient.create(
            payload_model_name=PAYLOAD_MODEL % GOREV3_PICKUP_TARGET_COLOR,
            vehicle_model_name=VEHICLE_MODEL_NAME,
            pose_monitor=gz_pose_monitor)
        warn_if_uncalibrated(logger)
        payload_manager = PayloadManager(DualPayloadBackend(
            real=RealPayloadBackend(real_flight.drone.action),
            sim=GazeboPayloadBackend(
                gz_hook_client,
                payload_model_name=PAYLOAD_MODEL % GOREV3_PICKUP_TARGET_COLOR,
                vehicle_model_name=VEHICLE_MODEL_NAME,
        ...
        pickup_phase = Gorev3PickupPhase(flight, camera, detector, payload_manager, position_store,
