...
class IFlightBackend(ABC):
    ...
    async def goto_position_ned_and_hold(self, north_m: float, east_m: float, down_m: float,
        ...
        """Bir setpoint'i duration_s boyunca tekrar tekrar gönderir (Görev 3
        Rapor, operatör revizyonu 2026-08-13). PX4 ~500ms setpoint'siz
        kalırsa Offboard'dan çıkar (bkz. hold_position/go_to_and_center'daki
        aynı BUG FIX) -- goto_position_ned'i tek seferlik çağırıp ardından
        asyncio.sleep() yapmak (Görev 3 fazlarının önceki, tamamen simüle
        edilmiş haliydi) bu yüzden gerçek uçuşta Offboard'dan düşerdi. Çok
        adımlı Görev 3 hareketleri (yaklaşma, geri çekilme, transit) artık
        bunu kullanır.

        north_m/east_m: goto_position_ned'in aksine MUTLAK DEĞİL -- çağrı
        anındaki mevcut yatay konuma göre GÖRELİ ofsettir (kök neden
        düzeltmesi, 2026-08-21; bkz. mavsdk_backend_base.py implementasyon
        notu). down_m/yaw_deg mutlak kalır."""
        pass
