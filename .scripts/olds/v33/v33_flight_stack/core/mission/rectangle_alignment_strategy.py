...
class RectangleAlignmentStrategy(IPayloadVisibilityStrategy):
    """DİNAMİK HEDEF (2026-08-24): aranan dikdörtgen artık constructor'da
    verilir. Eskiden `KIRMIZI_DIKDORTGEN` hardcode'du -- bu, Görev 3'ün her
    zaman Mavi Altıgen'e döneceği (kaldırılan) invaryanta dayanıyordu.
    Kırmızı Üçgen önce tamamlanırsa aranması gereken MAVI_DIKDORTGEN'dir.

    Varsayılan geriye dönük uyumluluk için KIRMIZI_DIKDORTGEN'dir."""

    def __init__(self, rectangle_shape: str = "KIRMIZI_DIKDORTGEN"):
        self.rectangle_shape = rectangle_shape

    async def locate_target(self, detector, camera_frame: np.ndarray) -> Detection:
        """Hedef dikdörtgeni bu karede arar. Bulunamazsa RAISE eder --
        ...
        for d in detections:
            if d.shape_type == self.rectangle_shape:
                return d
        raise RuntimeError(f"{self.rectangle_shape} bu karede bulunamadi")
