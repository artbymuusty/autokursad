...
class Gorev3Orchestrator:
    ...
    async def run(self) -> bool:
        ...
        # PHASE 11: transport'un donusu eskiden YOK SAYILIYORDU (run() zaten
        # None donuyordu), yani tasima gorevi hicbir kosulda dusuremiyordu.
        # Artik yuk kaybi/siralama hatasi burada gorevi temiz sekilde durdurur.
        transport_ok = await self.transport_phase.run()
        if not transport_ok:
            logger.error("GOREV 3 FAZ 2 (TASIMA) BASARISIZ")
            self.context.transition_to(MissionPhase.MISSION_FAILED, reason="gorev3_transport_failed")
            self._publish("GOREV3_PHASE_FAILED", "transport", severity=Severity.CRITICAL,
