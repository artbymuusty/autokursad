...
class Gorev2Orchestrator:
    ...
    async def _search_and_engage_loop(self) -> None:
        ...
        # ÇIPA (2026-08-24): döngü iki payload BIRAKILANA kadar sürer --
        # eskiden iki hedef KAYDEDİLENE kadardı. Dinamik akışta kayıt ve
        # bırakma aynı iterasyonda olduğu için ikisi arasındaki fark yalnızca
        # bir bırakma başarısız olursa ortaya çıkar; o durumda arama sürer.
        while (not self.interlock.both_released()
            ...
            for d in candidates:
                ...
                # DİNAMİK SIRA (2026-08-24, operatör kararı): hedef
                # kilitlendiği ANDA yükü bırakılır. 2026-08-13 revizyonunun
                # "Mission ASLA bırakma yapmaz, ikisini de bulana kadar
                # Route'a dön" kuralı bilinçli olarak TERSİNE ÇEVRİLDİ --
                # V33 spec md.6/11: hangi şekil önce kilitlenirse o işlenir.
                #
                # ADR-009 PX4-STABILITY KISITI KORUNDU (kritik):
                # O revizyonun asıl teknik gerekçesi Mission resume'un PX4'ü
                # kilitlemesiydi (ölçüm: 0.11 s aralıklı resume PX4'ü
                # tamamen kilitledi, 5.04 s'de 4. resume sessizce yok
                # sayıldı, hayatta kalan oturumda aralık 14.39 s).
                # Bu akış resume SAYISINI ARTIRMIYOR: eskiden de şimdi de
                # tam BİR resume var (ilk hedeften sonra). Değişen yalnızca
                # bırakmanın NE ZAMAN yapıldığı -- zaten var olan Offboard
                # penceresinin içinde, araç hedefin üstündeyken. Dahası iki
                # `_navigate_to_recorded` geri-uçuşu ortadan kalkıyor.
                # _resume_mission_route()'un kendi 15 s aralık + MISSION-mod
                # teyidi guard'ına DOKUNULMADI.
                #
                # Araç şu an hedefin üstünde kilitli, o yüzden navigate=False.
                released = await self.sequencer.execute_payload_mission_for(
                    selected.shape_type, navigate=False)
                self._publish_payload_sync()
                logger.info("%s yuku birakildi (dogrulama=%s). Tamamlanan: %d/2",
                            selected.shape_type, released,
                            int(self.interlock.payload_1_released) + int(self.interlock.payload_2_released))

                # ÇIPA DEĞİŞTİ (2026-08-24): Search Phase artık "iki hedef
                # kaydedildi" ile değil, "iki payload BIRAKILDI" ile biter.
                # Guard'ın kendisi (resume kalıcı olarak yasak) aynen duruyor.
                if self.interlock.both_released():
                    logger.info("SEARCH COMPLETE: iki payload da birakildi. "
                    ...
                    self._publish("SEARCH_WAYPOINTS_CANCELLED",
                                  data={"reason": "both_payloads_released_remaining_waypoints_ignored"})
                    break  # exits the for-loop; while-loop condition is now also False

                # Tek hedef işlendi -- operatörün QGC rotasına dönüp
                # diğerini aramaya devam et.
                self.context.transition_to(MissionPhase.SEARCHING, reason="single_target_processed_resuming_route")
                await self._resume_mission_route()
        ...
        # TELAFİ YOLU (2026-08-24): dinamik akışta bırakma zaten kilitlenme
        # anında yapıldı, bu yüzden execute_all() normalde HİÇBİR ŞEY yapmaz.
        # Yalnızca bir hedef kaydedilip yükü herhangi bir sebeple
        # bırakılmadan arama bittiyse devreye girer. Sabit 1->2 sırası
        # kaldırıldı (bkz. gorev2_fsm.py).
        await self.sequencer.execute_all()
        self._publish_payload_sync()

        if self.interlock.both_released():
            logger.info("Gorev 2 tamamlandi (iki payload da birakildi, tespit sirasina gore).")
            self.context.transition_to(MissionPhase.GOREV2_COMPLETE)
            self._publish("GOREV2_COMPLETE")
        else:
            # Buraya ulasilmasi beklenmez: dongu zaten both_released() olana
            # kadar surer ve execute_all() kalanlari telafi eder. Yine de
            # sessizce yutulmaz, gercek bir basarisizlik olarak islenir.
    ...
    async def _center_with_retries(self, shape_type: str, current_alt: float) -> bool:
        ...
        for attempt in range(1, CENTERING_MAX_ATTEMPTS_PER_TARGET + 1):
            ...
            if await self.centering.go_to_and_center(shape_type):
                return True
