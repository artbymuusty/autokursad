...
class HSVContourDetector(IDetector):
    ...
    @staticmethod
    def _detection_colour(shape_type: str) -> str:
        """KIRMIZI_* -> red, MAVI_* -> blue. Both come from the SAME mask
        this detector already keyed them by (red_mask -> KIRMIZI_*,
        blue_mask -> MAVI_*), so this is just reading that back off the
        name -- not a new colour source of truth."""
        return "red" if shape_type.startswith("KIRMIZI") else "blue"
    ...
    def _overlaps_committed(rect: Detection, committed: List[Detection]) -> bool:
        ...
        committed triangle/hexagon of the SAME colour -- i.e. it is that
        same blob seen through a different vertex count (PHASE 13 D2c,
        measured 2026-08-17: one red contour approximating to both 3 and 4
        vertices), not a second, genuinely different object.

        F3 fix (2026-08-20): the colour check was missing. A dropped
        payload is a real, differently-coloured rectangle that Görev 3
        deliberately places on top of the arena target it was aimed at
        (ADR-011: payloads are cross-coloured with their target on purpose,
        e.g. RED payload on the BLUE hexagon) -- its centre falls inside
        the hexagon's bbox by design, not by mis-approximation. Without the
        colour check this suppressed a real KIRMIZI_DIKDORTGEN every time
        it landed on MAVI_ALTIGEN, which is exactly when Görev 3 needs it
        (root-caused via live SITL + a passive Gazebo pose trace, not
        guessed). Same-colour suppression (the reason this function exists)
        is unchanged: it only ever fires between shapes from the same mask,
        same as before."""
        cx, cy = rect.center_px
        rect_colour = HSVContourDetector._detection_colour(rect.shape_type)
        for det in committed:
            if HSVContourDetector._detection_colour(det.shape_type) != rect_colour:
                continue
