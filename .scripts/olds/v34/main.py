...
def parse_args():
    ap = argparse.ArgumentParser(description="v34 Modular Autonomy System")
...
def main():
    ...
    if args.camera_only:
        ...
        prev_time = time.time()
        frame_count = 0
        cv2.namedWindow("V34 Camera Test", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("V34 Camera Test", DEBUG_WINDOW_WIDTH, DEBUG_WINDOW_HEIGHT)
        ...
        try:
            while True:
                ...
                cv2.imshow("V34 Camera Test", frame)
    ...
    if not mavsdk_ok:
        ...
        print(f"[INIT]   then re-run ./run_mission_v34.")
        sys.exit(1)
