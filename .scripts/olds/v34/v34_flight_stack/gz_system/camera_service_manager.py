"""
Ported from V30/V31 (.scripts/olds/v34/camera_manager.py), adapted to:
  ...
  - anchor PID/log file paths to this module's own directory instead of the
    caller's cwd, since v34_flight_stack's entrypoints are invoked from
    v34/ (via run_mission_v34_*.sh's `cd "$(dirname "$0")"`), not from
    inside gz_system/.
...
try:  # normal import when v34_flight_stack is on PYTHONPATH (mission entrypoints)
