...
async def test_nudge_forward_streams_then_stops():
    ...
    # distance_m=0.25 (not the production 0.10) so the streamed duration
    # (0.25/0.5=0.5s -> 5 ticks) leaves the rate limiter (2026-08-29:
    # SETPOINT_MAX_DELTA_V_M_S 0.15) enough ticks to actually reach 0.5 m/s
    # (0.15 -> 0.30 -> 0.45 -> 0.50) before the stream ends. Real production
    # usage (payload_release.py: nudge_forward(PAYLOAD_FINAL_FORWARD_M),
    # i.e. distance_m=0.10 at the default speed_m_s=0.3) already clears this
    # comfortably (duration 0.33s = ~3 ticks, target reached in 2) and is
    # unaffected either way.
    await controller.nudge_forward(distance_m=0.25, speed_m_s=0.5)
