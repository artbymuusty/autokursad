...
def _feed(*detections) -> DetectionFeed:
    ...
    stale_after_s is widened because this feed is published ONCE, whereas
    the real one is republished every frame by the orchestrator's detection
    loop and therefore never goes stale mid-release. Once the mock actuator
    grew get_released_payload_pose, release_and_verify started paying the
    real PAYLOAD_FINAL_POSE_DELAY_S (2.0 s) before verifying -- and this
    one-shot feed expired inside it, reporting "marker not in view" for a
    marker that production would still have been seeing. Widening the window
    models the live loop; it does not relax what is asserted."""
