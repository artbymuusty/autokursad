...
clustered around the 0.28 m mount offset in force at the time. The mission
centres the CAMERA; the payload hangs to one side and lands there.

GEOMETRY UPDATE (commit 3f606696): both payloads were remounted from the
+-0.28 m side rails to +-0.035 m directly under the airframe, which is
precisely the fix this analysis argued for -- the systematic miss shrinks
with the mount arm. The frame convention and the sign rule are unchanged and
are still what these tests pin down; the MAGNITUDE now comes from the SDF
instead of a literal, and the 0.28 m flights stay as the historical
cross-check they always were.
"""
import math
...
from sdf_geometry import payload_mount_offset_m
...
def test_mount_vectors_are_in_the_px4_frd_frame_not_the_sdf_flu_frame():
    ...
    # The SDF reader performs exactly the FLU->FRD negation this test is
    # about, so agreement with it IS the frame assertion -- and it cannot go
    # stale when the model moves, which is how this test previously rotted.
    assert PAYLOAD_MOUNT_OFFSET_BODY_M["MAVI_ALTIGEN"] == pytest.approx(
        payload_mount_offset_m("red"), abs=1e-9)
    assert PAYLOAD_MOUNT_OFFSET_BODY_M["KIRMIZI_UCGEN"] == pytest.approx(
        payload_mount_offset_m("blue"), abs=1e-9)
    # The sign flip is the regression itself: red mounts to the RIGHT in FRD
    # even though the world SDF writes it as a negative (left) FLU y.
    assert PAYLOAD_MOUNT_OFFSET_BODY_M["MAVI_ALTIGEN"][1] > 0
    assert PAYLOAD_MOUNT_OFFSET_BODY_M["KIRMIZI_UCGEN"][1] < 0
...
def test_reported_offset_is_payload_to_target_not_camera_to_target():
    ...
    # The aim shift must equal the CURRENT mount arm, read from the SDF.
    mount_right = abs(payload_mount_offset_m("red")[1])
    assert abs(aim_dx) * alt / focal == pytest.approx(mount_right, abs=0.005)
    assert aim_dx != 0.0, "a zero aim shift means the correction is disabled"
...
def test_mount_offset_matches_the_measured_landing_scatter():
    ...
    Scored against the +-0.28 m mount those four drops were FLOWN on. The
    payloads have since moved to +-0.035 m (3f606696); re-scoring 0.28 m-era
    scatter against 0.035 m would be testing a measurement against hardware
    that did not produce it, which is what made this assertion fail."""
    measured = [0.350, 0.373, 0.357, 0.337]
    mount_when_measured = 0.28
    residuals = [math.sqrt(max(0.0, m ** 2 - mount_when_measured ** 2))
                 for m in measured]
    assert max(residuals) < 0.25, residuals


def test_shrinking_the_mount_arm_shrinks_the_systematic_miss():
    """The remount's whole justification, stated as an assertion: the
    systematic drop error is the mount arm, so moving the payloads under the
    airframe must reduce it by the same ratio.

    Corroborated in flight (2026-08-26 acceptance mission): with the
    +-0.035 m mount the red payload came to rest 6.0 cm from the hexagon
    centre, against the 33.7-37.3 cm scatter of the +-0.28 m era."""
    mount_now = abs(PAYLOAD_MOUNT_OFFSET_BODY_M["MAVI_ALTIGEN"][1])
    assert mount_now == pytest.approx(abs(payload_mount_offset_m("red")[1]))
    assert mount_now < 0.28
    measured_miss_now_m = 0.060
    assert measured_miss_now_m < min([0.350, 0.373, 0.357, 0.337])
