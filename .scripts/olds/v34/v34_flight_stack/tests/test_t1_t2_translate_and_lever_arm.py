...
T2: the camera is mounted FORWARD of base_link, but the pixel
back-projection was added to the VEHICLE's GPS, so every frozen estimate
landed the lever arm's length aft of the real target. When this was
diagnosed the camera sat at 0.35 m: predicted landing error
0.35 - 0.10 (the final nudge) = 0.25 m south, measured across four flights
as 0.252, 0.264, 0.290, 0.305 m south.

GEOMETRY UPDATE (commit 3f606696): the camera moved to +0.085 m and both
payloads were remounted under the airframe at +-0.035 m. The lever-arm
CORRECTION is unchanged and still required; only its magnitude shrank. The
tests below therefore assert the constant against the SDF rather than
against a literal, and keep the 0.35 m diagnosis as the historical
cross-check it always was -- pinning a live constant to retired hardware is
what let this file rot.
"""
import math
...
from sdf_geometry import camera_body_offset_m


# ------------------------------------------------------------------ T2

def test_camera_lever_arm_matches_the_vehicle_sdf():
    """Straight off the mono_cam include pose. If this drifts from the SDF
    the correction silently becomes a new bias.

    Read from the SDF, not written out here: this assertion used to carry
    the literal (0.35, 0.0) and went stale the moment the model changed."""
    forward, right = camera_body_offset_m()
    assert CAMERA_LEVER_ARM_BODY_M == pytest.approx((forward, right), abs=1e-9)
    # and it must be a real forward offset -- a zeroed lever arm would mean
    # the correction had been silently disabled rather than resized.
    assert forward > 0.0
...
def test_lever_arm_predicts_the_measured_north_residual():
    ...
    Evaluated against the geometry the four flights were FLOWN on (camera at
    0.35 m), because that is the configuration the residuals were measured
    in. Re-evaluating a 2026-08 measurement against 3f606696's 0.085 m
    camera would be comparing a result to hardware that did not produce it.
    """
    lever_arm_when_measured_m = 0.35
    nudge_forward_m = 0.10
    predicted = lever_arm_when_measured_m - nudge_forward_m
    measured = [0.252, 0.264, 0.290, 0.305]
    assert predicted == pytest.approx(0.25)
    assert abs(sum(measured) / len(measured) - predicted) < 0.05


def test_lever_arm_correction_scales_with_the_current_geometry():
    """The live constant must still predict a real, bounded bias: the same
    mechanism, resized. With the camera at 0.085 m the uncorrected estimate
    is ~8.5 cm aft -- an order of magnitude smaller than the 0.35 m era, and
    that shrinkage is the point of the remount."""
    forward, _right = camera_body_offset_m()
    assert CAMERA_LEVER_ARM_BODY_M[0] == pytest.approx(forward)
    assert 0.0 < forward < 0.35
