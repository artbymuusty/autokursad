"""
Required test scenarios from the operator's "V34 Flight Authority & Mission
...
LIFECYCLE CHANGE (operator, 2026-08-21) -- read before editing timings.
The orchestrator now releases a payload THE MOMENT its target is centred,
instead of deferring every release until the search phase has finished. That
deliberately inverts the older "Mission (Search Phase) ASLA yuk birakma
islemi yapmaz" rule, and it is the shorter, lower-error path: the vehicle is
already centred over the target, so it does not have to fly back to a
recorded GPS fix and re-converge.

Every AUTHORITY invariant in this file is unchanged and still asserted --
one flight authority, no start_mission after search completes, Offboard-only
payload work, correct interlock ordering. What changed is only WHEN the
release happens, so the scenarios that record a single target now pay for a
full staged descent inside the search loop. The time budgets below were
raised for that reason; none of the assertions were relaxed.
"""
import asyncio
import pytest
...
async def _run_bounded(orch, flight, end_after_s=0.4, timeout_s=120):
    """Mark the route finished shortly after start, then let the orchestrator
    wind down on its own.

    timeout_s is a DEADLOCK guard, not a performance budget: since the
    2026-08-21 lifecycle change a single recorded target drags a complete
    staged descent and release into this window (see the module docstring),
    so the old 15 s was tripping on real work rather than on a hang."""
    async def end_soon():
        await asyncio.sleep(end_after_s)
        flight._is_mission_finished = True
    await asyncio.wait_for(asyncio.gather(orch.run(), end_soon()), timeout=timeout_s)
...
async def test_2_blue_only_search_continues_mission_may_resume(tmp_path):
    ...
    # Recording MAVI_ALTIGEN now also RELEASES payload 1 in place, so this
    # scenario runs a full staged descent; same speed-up as TEST 4.
...
async def test_4_both_targets_found_search_completes_permanently_offboard_sole_authority(tmp_path):
    ...
    await asyncio.wait_for(orch.run(), timeout=180)
