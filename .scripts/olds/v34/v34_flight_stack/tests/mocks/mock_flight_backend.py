...
class MockFlightBackend(IFlightBackend):
    ...
    async def goto_position_ned(self, north_m: float, east_m: float, down_m: float, yaw_deg: float) -> None:
        ...
        # SAME GAP AS set_velocity_body BELOW, one level up (2026-08-27).
        # A NED goto that moves _ned_pos but leaves _global_pos altitude at
        # its initial 15 m was harmless while only the horizontal position was
        # read back. Görev 3 Faz 1 now closes a VISION loop whose scale comes
        # from get_global_position()[2], so a frozen altitude made every
        # synthetic frame look like a 15 m hover: the payload projected to
        # 18 px, below any detection gate, and the phase correctly refused to
        # proceed -- a mock artefact presenting as a production failure.
        self._global_pos = (self._global_pos[0], self._global_pos[1], -down_m)
