...
class MavsdkBackendBase(IFlightBackend):
    ...
    def __init__(self, connection_string: str, publisher: EventPublisher = NULL_PUBLISHER):
        ...
        # ADR-010 R2 devami: mission_progress.total, indisi gecerli
        # araliga kirpabilmek icin saklanir (bkz.
        # get_current_mission_index).
        ...
        while True:
            try:
                async for progress in self.drone.mission.mission_progress():
                    ...
                    self._mission_total_items = progress.total
    ...
    async def get_current_mission_index(self) -> int:
        ...
        # BUG FIX (2026-08-21): ham deger dogrudan set_current_mission_item'a
        # veriliyordu. Rota bittiginde PX4 current == total bildiriyor (5
        # ogeli rotada 5), bu da gecerli indis araligi 0..total-1'in disi:
        #   set_current_mission_item basarisiz (INVALID_ARGUMENT; params: (5,))
        #   MISSION_RESUME_NOT_CONFIRMED: PX4 iki denemede de MISSION moduna
        #   donmedi -- arac muhtemelen HOLD'da kalacak, rota ilerlemeyecek
        # ve arama fazi orada oluyordu (olculdu: mission18 ve izlemeli kosu).
        # Gecerli araliga kirp; total henuz bilinmiyorsa ham degeri birak.
        if self._mission_total_items > 0:
            return max(0, min(self._mission_current_index, self._mission_total_items - 1))
