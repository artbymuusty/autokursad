...
class PayloadReleaseService:
    ...
    async def release_and_verify(self, shape_type: str, *,
        ...
        climb_back_alt_m: bırakma sonrası geri tırmanılacak irtifa. Varsayılan
        MISSION_ALTITUDE_M'dir çünkü NORMAL durumda çağıran taraf
        (Gorev2Orchestrator/PayloadMissionSequencer) bu tırmanışı TÜKETİR:
        aramaya kaldığı yerden devam eder ya da ikinci hedefe MISSION_ALTITUDE_M
        seviyesinden gider (Bölüm 4.2, 6, 9). İSTİSNA -- TERMİNAL (son) bırakma:
        payload 2'den sonra Görev 2 biter ve master_fsm doğrudan Görev 3'e
        devreder; Görev 3'ün ilk komutu GOREV3_TRANSIT_ALTITUDE_M = 1.5 m'dir,
        yani 15 m'ye tırmanmak ölçülen ~13.2 m tırmanış + ~13.2 m alçalma /
        ~9 s'lik saf israftır. O çağrı için
        GOREV2_FINAL_RELEASE_CLIMB_ALTITUDE_M geçilir (bkz. parameters.py --
        emniyet payı hesabı orada)."""
        ...
        # forward -- always climb back before returning, regardless of
        # verification outcome, so the caller (Gorev2Orchestrator) can
        # unconditionally resume search or engage the second target from
        ...
        #
        # The DEFAULT is still MISSION_ALTITUDE_M and the reasoning above is
        # unchanged for it. The exception is the TERMINAL drop: after payload
        # 2 nobody consumes that altitude -- Görev 2 ends and Görev 3's first
        # command is GOREV3_TRANSIT_ALTITUDE_M = 1.5 m -- so that one caller
        # passes GOREV2_FINAL_RELEASE_CLIMB_ALTITUDE_M instead (measured
        # waste: ~13.2 m up + ~13.2 m down, ~9 s; see parameters.py).
