...
class PhaseTransition:
    """One MISSION_PHASE_CHANGED, recorded verbatim.

    core/mission/context.py::transition_to() is the single mutation point for
    mission phase and publishes exactly one of these per transition, so this
    is the mission's chronological record -- NOT something re-derived by
    sampling `phase` per render tick (at the dashboard's 10 Hz a single 17 s
    phase would otherwise produce ~170 identical rows).

    `duration_s` is how long `from_phase` lasted (the event's
    `previous_phase_duration_s`), not how long `to_phase` will last -- that
    one is only knowable once the NEXT transition arrives, which is exactly
    why it is carried on the transition that ends it.
    """
    from_phase: str = ""
    to_phase: str = ""
    reason: str = ""
...
class MissionSnapshot:
    ...
    # Deliberately SEPARATE from recent_events rather than filtered out of it.
    # recent_events is an 80-entry ring dominated by VEHICLE_TELEMETRY and
    # WATCHDOG traffic -- a real 600 s run logged 4274 events of which only 79
    # were phase changes, and the surviving last-80 window covered just 46.8 s.
    # Sharing that buffer would let routine telemetry noise evict the mission's
    # own progress record within a minute.
