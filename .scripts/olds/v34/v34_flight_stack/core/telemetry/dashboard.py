...
PROGRESS_COLUMN_WIDTH = 400
DISPLAY_SCALE = 1.2

# Line pitch of the progress list, matching _draw_timeline's -- at a 960 px
# camera panel that is ~59 rows, which is comfortably more than the ~79
# transitions a full run produces once repeats are collapsed.
PROGRESS_LINE_PITCH_PX = 15

# Short operator-facing labels for the phase enum. This is a RENAMING only --
# it neither reorders anything nor implies steps that have not happened. A
# phase with no entry here renders its raw enum value, so a newly added phase
# degrades to "visible but verbose" rather than "invisible".
_PHASE_LABEL = {
    "MISSION_INIT": "INIT",
    "CONNECTING": "CONNECTING",
    "ARMING": "ARMING",
    "TAKEOFF": "TAKEOFF",
    "CLIMB_TO_ALTITUDE": "CLIMB TO ALT",
    "CHECKPOINT_SAVE": "CHECKPOINT SAVE",
    "MISSION_ROUTE_CONFIRM": "ROUTE CONFIRM",
    "MISSION_UPLOAD": "ROUTE UPLOAD",
    "MISSION_START": "ROUTE START",
    "SEARCHING": "SEARCHING",
    "TARGET_TRACKING": "TARGET TRACKING",
    "SWITCH_TO_OFFBOARD": "TO OFFBOARD",
    "GOTO_TARGET_CENTERING": "CENTERING",
    "HOVER_CONFIRM": "HOVER CONFIRM",
    "GPS_SAVE": "GPS SAVE",
    "PAYLOAD_DECISION": "PAYLOAD DECISION",
    "PAYLOAD_RELEASE": "PAYLOAD RELEASE",
    "PAYLOAD_VERIFY": "PAYLOAD VERIFY",
    "RETURN_TO_SECOND_TARGET": "TO 2ND TARGET",
    "SEARCH_COMPLETE": "SEARCH COMPLETE",
    "GOREV2_COMPLETE": "GOREV 2 DONE",
    "GOREV3_START": "GOREV 3 START",
    "GOREV3_RUNNING": "GOREV 3 RUNNING",
    "RETURN_TO_CHECKPOINT": "TO CHECKPOINT",
    "LANDING": "LANDING",
    "MISSION_COMPLETE": "MISSION COMPLETE",
    "MISSION_FAILED": "MISSION FAILED",
    "MISSION_ABORTED": "MISSION ABORTED",
    "MISSION_TIMEOUT": "MISSION TIMEOUT",
}

# The terminal phases that are FAILURES, taken from phase.py's own set rather
# than re-listed here -- these are exactly the ones MissionContext.transition_to
# publishes at Severity.CRITICAL.
_FAILURE_PHASES = frozenset(
    p.value for p in TERMINAL_PHASES if p is not MissionPhase.MISSION_COMPLETE)

# A transition into a perfectly normal phase can still be BAD news, and the
# only place that shows is the reason string ("centering timed out",
# "retrying in place"). Those rows are drawn in the warning colour so a
# 13-retry search loop is visible as trouble rather than as 13 tidy steps.
_WARN_REASON_WORDS = ("timeout", "timed_out", "timed out", "fail", "abort",
                      "retry", "retrying", "lost", "giving up")

# ASCII only, deliberately: cv2.putText's Hershey fonts have no glyphs for
# U+2713 / U+2192 and render them as "?" -- a check mark that displays as a
# question mark is worse than no marker at all.
_MARK_DONE = "[x]"
_MARK_CURRENT = "[>]"
_MARK_FAILED = "[!]"
...
def _mmss(seconds: float) -> str:
    """Mission-relative MM:SS. Clamped at zero so a transition whose ts
    predates started_at (clock skew, a replayed log) renders 00:00 instead of
    a negative time."""
    try:
...
def _phase_label(phase_value: str) -> str:
    """Short operator label, falling back to the raw enum value with its
    underscores opened up. Turkish phase names (GOREV2_COMPLETE, ...) are
    kept as-is -- this is a Turkish mission and the operator reads them."""
    if not phase_value:
        return "(unknown)"
    return _PHASE_LABEL.get(phase_value, phase_value.replace("_", " "))


def _fit_text(text: str, max_px: int, scale: float = 0.36, thick: int = 1) -> str:
    """Truncate `text` to roughly `max_px` wide, with a ".." marker so a cut
    reason never reads as a complete one. Estimated from ONE measurement
    rather than a shrink-and-remeasure loop -- this runs per row, per frame,
    at 10 Hz."""
    ...
    if w <= max_px:
        return text
    per_char = max(1.0, w / float(len(text)))
    keep = max(0, int(max_px / per_char) - 2)
    return (text[:keep] + "..") if keep else ""


@dataclass
class ProgressRow:
    """One rendered line of the MISSION PROGRESS panel: a run of consecutive
    transitions that were identical in both destination AND reason."""
    to_phase: str
    reason: str
    count: int
    first_ts: float
...
def collapse_phase_history(transitions: Sequence[PhaseTransition]) -> List[ProgressRow]:
    """Run-length encode the phase history on (to_phase, reason).

    Keyed on BOTH fields on purpose. transition_to() does not guard
    phase == previous, so the same phase can legitimately be re-entered back
    to back; when that happens the `reason` is the only thing distinguishing
    the two, and merging across differing reasons would erase the very
    information that explains the repeat.

    Run-length, not a global tally: only ADJACENT identical rows merge, so a
    phase revisited later in the mission gets its own line and the
    chronological order is preserved exactly. Nothing is ever dropped -- a
    merged run keeps its count ("SEARCHING x13") and both endpoints' times.
    """
    rows: List[ProgressRow] = []
    for tr in transitions:
        if rows and rows[-1].to_phase == tr.to_phase and rows[-1].reason == tr.reason:
            rows[-1].count += 1
            rows[-1].last_ts = tr.ts
            continue
        rows.append(ProgressRow(to_phase=tr.to_phase, reason=tr.reason,
                                count=1, first_ts=tr.ts, last_ts=tr.ts))
    return rows
...
class MissionOpsDashboard:
    def __init__(
        ...
        progress_col_width: int = PROGRESS_COLUMN_WIDTH,
    ):
        ...
        self.progress_col_width = progress_col_width
        self._running = False
    ...
    def _render(self, snap: MissionSnapshot, sample: Optional[FrameSample]) -> None:
        ...
        # Every column is built to the CAMERA panel's height, not to a
        # constant: np.hstack requires exact agreement, and the camera panel
        # is 960 px live but 480 px while it is still the placeholder.
        camera_img = self._build_camera_panel(sample, snap)
        telemetry_img = self._build_telemetry_column(snap, height=camera_img.shape[0])
        progress_img = self._build_progress_column(snap, height=camera_img.shape[0])
        combined = np.hstack((camera_img, telemetry_img, progress_img))
    ...
    # ------------------------------------------------------------------
    # mission progress column (far right)
    # ------------------------------------------------------------------
    def _build_progress_column(self, snap: MissionSnapshot, height: int) -> np.ndarray:
        """Chronological "what has this mission actually done" list.

        Reads snap.phase_history, which the aggregator fills from
        MISSION_PHASE_CHANGED -- one event per transition, published by the
        single mutation point (MissionContext.transition_to). It is
        deliberately NOT derived by sampling snap.phase each render tick: at
        10 Hz one 17 s phase would append ~170 identical rows.

        Pure pixel work (rectangle/putText/np.full) -- ADR-006: not one cv2
        GUI call anywhere on this path, so it stays safe to run on the
        dashboard thread while Cocoa owns the paint on the main thread.
        """
        img = np.full((height, self.progress_col_width, 3), COL_BG, dtype=np.uint8)
        w = img.shape[1]
        panel_h = max(60, height - 12)
        body_y = self._panel(img, 6, w - 6, 6, panel_h, "MISSION PROGRESS")

        rows = collapse_phase_history(snap.phase_history)

        clock = f"T+{_mmss(snap.elapsed_s)}"
        self._text(img, clock, 14, body_y, COL_TEXT, 0.5, 1)
        steps = f"{len(snap.phase_history)} steps" if rows else "no steps yet"
        (sw, _), _ = cv2.getTextSize(steps, FONT, 0.4, 1)
        self._text(img, steps, w - 14 - sw, body_y, COL_TEXT_DIM, 0.4, 1)
        cv2.line(img, (14, body_y + 9), (w - 14, body_y + 9), COL_PANEL_BORDER, 1)

        yy = body_y + 24
        bottom = 6 + panel_h - 8
        self._draw_progress_rows(img, snap, rows, yy, bottom)
        return img

    def _draw_progress_rows(self, img, snap: MissionSnapshot, rows: List[ProgressRow],
                            y: int, bottom: int) -> None:
        w = img.shape[1]
        pitch = PROGRESS_LINE_PITCH_PX
        max_lines = max(1, (bottom - y) // pitch)

        if not rows:
            # No transition has been published yet. The mission genuinely
            # starts in MISSION_INIT without one, so say that rather than
            # fabricating a first step -- and never list phases that have not
            # happened: the route through the state machine is data-dependent
            # (retries, Görev 3 branch) and is not knowable in advance.
            self._text(img, "awaiting first phase transition", 14, y, COL_TEXT_DIM, 0.4, 1)
            self._text(img, f"current: {_phase_label(snap.phase.value)}", 14, y + pitch,
                       COL_TEXT_DIM, 0.4, 1)
            return

        # Render only the TAIL that fits, newest at the bottom. When rows are
        # dropped, say how many -- an operator must never mistake a truncated
        # list for the whole mission.
        overflow = max(0, len(rows) - max_lines)
        if overflow:
            overflow += 1                       # the note itself costs a line
            self._text(img, f"... {overflow} earlier steps", 14, y, COL_TEXT_DIM, 0.36, 1)
            y += pitch
            rows = rows[overflow:]
        ...
        for i, row in enumerate(rows):
            ...
            failed = row.to_phase in _FAILURE_PHASES
            reason_l = row.reason.lower()

            if failed:
                mark, color = _MARK_FAILED, COL_BAD
            elif is_last:
                mark = _MARK_CURRENT
                color = COL_WARN if any(k in reason_l for k in _WARN_REASON_WORDS) else COL_ACCENT
            else:
                mark = _MARK_DONE
                if row.to_phase == MissionPhase.MISSION_COMPLETE.value:
                    color = COL_GOOD
                elif any(k in reason_l for k in _WARN_REASON_WORDS):
                    color = COL_WARN
                else:
                    color = COL_TEXT

            head = f"{mark} {_mmss(row.first_ts - snap.started_at)} {_phase_label(row.to_phase)}"
            self._text(img, head, 14, y, color, 0.38, 1)
            (hw, _), _ = cv2.getTextSize(head, FONT, 0.38, 1)

            limit = w - 14
            if row.count > 1:
                tag = f"x{row.count}"
                (tw, _), _ = cv2.getTextSize(tag, FONT, 0.38, 1)
                self._text(img, tag, w - 14 - tw, y, COL_ACCENT, 0.38, 1)
                limit -= tw + 8

            # The reason is what distinguishes one re-entry of a phase from
            # the next, so it is shown whenever there is room -- dim, so the
            # phase names still read as the primary column.
            reason_x = 14 + hw + 8
            if row.reason and reason_x < limit - 24:
                self._text(img, _fit_text(row.reason, limit - reason_x, 0.34),
                           reason_x, y, COL_TEXT_DIM, 0.34, 1)
            y += pitch
