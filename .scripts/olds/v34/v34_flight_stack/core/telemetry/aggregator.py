...
from core.telemetry.snapshot import CenteringState, MissionSnapshot, PhaseTransition, TrackState

RECENT_EVENTS_MAX = 80

# Phase transitions are kept in their own, much deeper buffer than
# recent_events. A full 600 s Görev 2/3 run produces ~79 of them (repeats
# included), so 400 holds several runs' worth and the cap only exists so a
# pathological transition storm cannot grow without bound.
PHASE_HISTORY_MAX = 400
...
class RuntimeStateAggregator:
    ...
    def on_event(self, event: Event) -> None:
        with self._lock:
            ...
            if event.code == "MISSION_PHASE_CHANGED":
                ...
                # Chronological progress record. Appended here, and ONLY
                # here, because transition_to() publishes exactly one of
                # these per transition -- one event in, one row out. Repeats
                # are kept as-is: transition_to() does not guard
                # phase == previous, so a re-entered phase is a real mission
                # event (13 SEARCHING re-entries on centering timeouts in the
                # reference run) and collapsing belongs to the renderer, not
                # to the record.
                self._phase_history.append(PhaseTransition(
                    from_phase=str(event.data.get("from_phase", "") or ""),
                    to_phase=str(event.data.get("to_phase", "") or ""),
                    reason=str(event.data.get("reason", "") or ""),
                    ts=event.ts,
                    duration_s=float(event.data.get("previous_phase_duration_s") or 0.0),
                ))
                if len(self._phase_history) > PHASE_HISTORY_MAX:
    ...
    def snapshot(self) -> MissionSnapshot:
        ...
        with self._lock:
            ...
            snap.phase_history = list(self._phase_history)
