...
from gz_system.gz_payload_actuator import HOOK_WINCH_EXTEND_M, hook_payout_m
...
class Gorev3PickupPhase:
    ...
    async def run(self) -> bool:
        ...
        _set_winch = getattr(self.actuator, "set_winch", None)
        if _set_winch is not None:
            _payout_alt = await self._current_alt_m()
            _payout = hook_payout_m(GOREV3_DESCENT_ALTITUDE_M)
            logger.info("Vinc salinacak (%.3f m) -- ARAC HALA %.2f m'de, kanca "
                        "serbest asili kalacak.", _payout,
                        _payout_alt if _payout_alt is not None else float("nan"))
            await _set_winch(_payout)
        ...
        _hold = asyncio.create_task(self.flight.goto_position_ned_and_hold(
            _hn, _he, -GOREV3_DESCENT_ALTITUDE_M, aligned_yaw, PICKUP_HOLD_S))
        ...
        self._publish("GOREV3_PICKUP_STEP", "pickup_attempt_start",
                      data={"altitude_m": (round(_pick_alt, 3)
                                           if _pick_alt is not None else None)})
        try:
            picked = await self.actuator.activate_pickup_mechanism(altitude_m=_pick_alt)
        finally:
            _hold.cancel()
            try:
                await _hold
