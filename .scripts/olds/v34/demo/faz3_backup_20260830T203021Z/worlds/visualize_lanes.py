#!/usr/bin/env python3
"""KURSAD40 v34 -- iki-lane test ortami + QGC rotalari gorsellestirici.

NEDEN VAR
---------
`generate_test_lanes.py` sekilleri HER CALISTIRMADA yeniden konumlandiriyor,
QGC rotalari ise (`Documents/QGroundControl Daily/Missions/lane_*.plan`)
elle yuklenen ayri dosyalar. Ikisinin AYNI dunyada birbiriyle tutarli olup
olmadigini -- sekiller rotanin kamera izine giriyor mu, lead-in/run-out
bacaklari lane disina mi tasiyor, U donusu gercekten yumusak mi --
...
-----------------------------------------
  sekiller  : default.sdf, KURSAD_LANE_{A,B}_START/END bloklari arasindaki
...
BILINEN DURUM -- 1WAY'in RTL'i lane uzerinden geri doner
---------------------------------------------------------
Gorseldeki 1WAY rotalari son waypoint'te (Xc, +120) biter ve oradan RTL
devreye girer. Home spawn'da (25, 0) oldugu icin RTL yolu lane'in
UZERINDEN guneye dogru gecer -- yani drone tarama bandini ikinci kez,
bu sefer 30 m'de (rc.mc_defaults: RTL_RETURN_ALT 30) katetmis olur.
Olculen: RTL duz hatti lane ayak izinde Y=100'den Y=48'e kadar, yani
bandin 52 m'si boyunca kaliyor (her iki lane'de de ayni).

Bu BILEREK boyle birakildi (2026-08-30, operator karari):
  * Gorev zaten bitmistir; tarama pasi tamamlanmis, RTL sadece eve donus.
  * Kacinmanin tek yolu RTL'i birakip lane disindan elle bir donus rotasi
    cizmek olurdu; bu da "rota tanimi QGroundControl'un isi" mimari
    kuralini bozmadan yapilamaz ve gorev suresini uzatir.
  * 30 m'den gorulen kare, 12 m'dekinin ~1/2.5 olceginde -- tespit
    pipeline'i gorev sonunda durmuyorsa bu ikinci gecisi ayni sekiller
    icin DUSUK GUVENLI ikinci bir tespit serisi olarak gorebilir.

Yani: pipeline mission-end'de kayit almayi durduruyorsa hicbir etkisi
yok. Durdurmuyorsa, 1WAY loglarindaki "ayni sekil, ikinci kez, daha
kucuk" tespitlerin kaynagi budur -- bir hata degil.

2WAY'de bu durum YOKTUR: rota (Xc-7.25, -20)'de, yani lane'in guneyinde
biter ve RTL oradan home'a (25, 0) dogrudan, lane'e girmeden gider.
...
OUT_PNG   = os.path.join(HERE, "lane_layout_preview.png")

# --- lane geometrisi (generate_test_lanes.py ile ayni) ----------------------
LANE_WIDTH, LANE_LENGTH, EDGE_MARGIN = 30.0, 100.0, 3.0
LANES = {"A": 0.0, "B": 50.0}

# --- FAZ 3 rota tasarim sabitleri -------------------------------------------
ALT        = 12       # gorev irtifasi (m, relative)
...
ARC_OFFSET = 13.0     # yay merkezi Y=100'un bu kadar kuzeyinde.
                      ...
                      # bassaydi fren tarama bandinin 6 m ICINDE (Y=94.06)
                      # baslardi. 13 m ile fren Y=102.06'da basliyor.

# --- gorsel stil ------------------------------------------------------------
RED, BLUE = "#ff0000", "#008cff"           # model.sdf <diffuse> degerleri
ROUTE_STYLE = {                            # (renk, cizgi stili, etiket)
    "A_1way": ("#e6ab02", "-",  "Lane A 1WAY"),
    "A_2way": ("#7570b3", "--", "Lane A 2WAY"),
    "B_1way": ("#d95f02", "-",  "Lane B 1WAY"),
    "B_2way": ("#1b9e77", "--", "Lane B 2WAY"),
}
...
def parse_shapes(path: str = WORLD) -> list[dict]:
    """KURSAD_LANE_{A,B} bloklari arasindaki <include>'lari oku.

    Yorum metnine degil, marker sinirlarina ve <uri>/<name>/<pose> etiketlerine
    dayanir -- generate_test_lanes.py her calistiginda bu bloklari yeniden
    yazdigi icin baska hicbir sey sabit degil."""
    txt = open(path).read()
    shapes = []
    for lane in ("A", "B"):
        blk = re.search(rf"KURSAD_LANE_{lane}_START(.*?)KURSAD_LANE_{lane}_END",
                        txt, re.S)
        if not blk:
            print(f"  ! UYARI: KURSAD_LANE_{lane} blogu default.sdf'te yok")
            continue
...
def design_routes() -> dict[str, list[tuple[float, float]]]:
    ...
    sx, sy = parse_spawn()
    out = {}
    for lane, cx in LANES.items():
        out[f"{lane}_1way"] = [
            (sx, sy),
            (cx, -LEADIN),
            (cx, LANE_LENGTH + LEADIN),
        ]
        cy = LANE_LENGTH + ARC_OFFSET
...
def plan_routes() -> dict[str, list[tuple[float, float]]]:
    ...
    for key in ROUTE_STYLE:
        p = os.path.join(MISSIONS, f"lane_{key}.plan")
        if not os.path.exists(p):
...
def scan_tracks(pts) -> list[float]:
    ...
    for (x1, y1), (x2, y2) in zip(pts, pts[1:]):
        if (abs(x2 - x1) < 0.05                       # dik (kuzey-guney) bacak
                ...
                and max(y1, y2) >= LANE_LENGTH - 0.5):  # ustunde bitiyor
...
def draw_scene(ax, shapes, spawn, routes, *, legend=False, arrows=True,
    ...
    for lane, cx in LANES.items():
        ...
        ax.add_patch(Rectangle((x0 + EDGE_MARGIN, EDGE_MARGIN),
                               LANE_WIDTH - 2 * EDGE_MARGIN,
                               LANE_LENGTH - 2 * EDGE_MARGIN,
    ...
    for key, (col, ls, lbl) in ROUTE_STYLE.items():
        ...
        if swath > 0:
            for tx in scan_tracks(pts):
                ax.add_patch(Rectangle((tx - swath / 2, 0), swath, LANE_LENGTH,
                                       facecolor=col, alpha=0.13, lw=0, zorder=2))
                ax.plot([tx - swath / 2] * 2, [0, LANE_LENGTH], color=col,
...
def draw_crosstrack(ax, shapes, routes, swath):
    ...
    rows = []
    for lane in ("A", "B"):
        rows += [(f"{lane}_1way", f"Lane {lane}  1WAY izi"),
                 (f"{lane}_2way", f"Lane {lane}  2WAY izleri")]

    for cx in LANES.values():                       # lane sinirlari
        ax.axvspan(cx - LANE_WIDTH / 2, cx + LANE_WIDTH / 2,
                   facecolor="#e8e8e8", edgecolor="#404040", lw=1.2, zorder=0)
        ax.axvspan(cx - LANE_WIDTH / 2 + EDGE_MARGIN,
                   cx + LANE_WIDTH / 2 - EDGE_MARGIN,
    ...
    for i, (key, label) in enumerate(rows):
        ...
        ax.text(-24, y, label, va="center", ha="left", fontsize=8.5)
    ...
    ax.text(-24, 0.35, "sekiller (X konumu)", va="center", ha="left", fontsize=8.5)
...
def main() -> int:
    ...
    for s in shapes:
        ...
        cx = LANES[s["lane"]]
        print(f"  Lane {s['lane']}  {s['name']:<16} X={s['x']:8.3f} Y={s['y']:8.3f}"
              f"  ({w:g}x{h:g} m)  merkeze uzaklik dX={s['x']-cx:+7.3f}")
    ...
    ax.set_title("KURSAD40 v34 -- Lane A/B yerlesimi + QGC rotalari\n"
    ...
    # --- zoom 1: Lane A U donusu ---
    draw_scene(az1, shapes, spawn, routes, arrows=False)
    az1.set_xlim(-14, 14)
    az1.set_ylim(96, 124)
    az1.set_title(f"ZOOM: Lane A U-donusu (R={ARC_R:g} m, {ARC_SEGS} segment)\n"
                  f"yay merkezi Y={LANE_LENGTH+ARC_OFFSET:g}, "
                  f"tarama bandi Y<={LANE_LENGTH:g}", fontsize=10)
    az1.axhline(LANE_LENGTH, color="crimson", lw=1.2, ls="--", zorder=9)
    az1.text(-13.4, LANE_LENGTH + 0.4, "tarama bandi ust siniri Y=100",
    ...
    az1.add_patch(Arc((0.0, LANE_LENGTH + ARC_OFFSET), 2 * ARC_R, 2 * ARC_R,
                      theta1=0, theta2=180, edgecolor="black", lw=1.0, ls=":",
                      zorder=9))
    az1.text(0, LANE_LENGTH + ARC_OFFSET + 0.6,
