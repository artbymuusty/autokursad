...
def _lat_summary(lat_all, lat_gate_ok):
    ...
    lat_all      : penceredeki HER ornegin yanal ofseti (mm)
    lat_gate_ok  : yanal DISINDAKI butun kapi terimlerini gecen orneklerin
                   yanal ofseti (mm) -- "yalnizca yanal yuzunden reddedilenler"

    n_le_10_25 / n_le_23_25 ikilisi, kapi sikilasmasinin etkisini fiziksel
    hapsin etkisinden ayirmayi saglar: ayni kosu, iki esik. lat_gate_ok
    uzerinden bakildiginda bunlar dogrudan "eski kapi acilir miydi / yeni
    kapi acilir mi" sorusunun cevabidir.
    """
    def q(xs, f):
        if not xs:
            return None
        s = sorted(xs)
        return round(s[min(len(s) - 1, int(f * len(s)))], 2)
    out = {
        "n": len(lat_all),
        "n_gate_ok_except_lateral": len(lat_gate_ok),
        "min_mm": round(min(lat_all), 2) if lat_all else None,
        "p10_mm": q(lat_all, 0.10), "p50_mm": q(lat_all, 0.50),
        "p90_mm": q(lat_all, 0.90),
    }
    for label, xs in (("all", lat_all), ("gate_ok", lat_gate_ok)):
        out[f"{label}_le_10_25"] = sum(1 for v in xs if v <= 10.25)
...
class GzPayloadActuator(IPayloadActuator):
    ...
    async def _await_seating(self, color: str, timeout_s: float):
        ...
        # dayansin.
        offs = []
        # SALT OLCUM: yuva collision'i eklenince yanal kapi 23.25 -> 10.25 mm
        # sikilasti. Bunun kapinin acilma sikligina net etkisi ONGORULEMEZ,
        # cunku iki zit etki var: (a) daha dar kapi tek basina daha az acilir,
        # (b) cep duvari burnu fiziksel olarak 10.25 mm icinde tutuyor, yani
        # dar araliktaki ornek ORANI artabilir. Ikisini ayirmak icin her
    ...
    async def extend_winch_for(self, altitude_m, deck_height_m: float = HOOK_RECEIVER_DECK_HEIGHT_M) -> bool:
        """Vinci VERILEN IRTIFAYA gore hesaplanan salima surer.
    ...
    async def _retry_realign(self, on_retry, attempt: int) -> None:
        ...
        Bu noktada vinc CEKILI, yani kanca HAVADA asili -- mekanizma 2'de
        duzeltme dongusunun calistigi KANITLI rejim. Onceden bu firsat
        kaciriliyordu ve her deneme, bir oncekinin biriktirdigi hatadan
        basliyordu. Olculdu (Y1 izi, 2026-08-31): bir denemeden sonra kanca
        yuvadan ~47 mm otede duruyor -- inis 1.9 mm + temas sicramasi
        23.0 mm + temas sonrasi kayma 22.8 mm. Yeniden hizalama her denemeyi
        8-9 mm'den baslatir.

        Ucus kontrolu bu katmanda YOK; bu yuzden geri cagri. Aktuator
        ucustan bagimsiz kalir, hizalamayi gorev katmani yapar. on_retry
        verilmezse davranis eskisiyle BIREBIR aynidir.
        """
        if on_retry is None:
            return
        # SON DENEMEDEN SONRA HIZALAMA YAPMA. Dongu son denemede de bu yola
        # giriyor (basarisizlik -> vinc cek -> continue -> dongu biter), ama
        # arkasindan bir deneme GELMIYOR. Olculdu (2026-08-31, 6 kosunun
        # 3'u): kosu basina ~10-20 s bosa gidiyordu.
        if attempt >= HOOK_PICKUP_ATTEMPTS:
            logger.info("[HOOK] son deneme (%d/%d) -- yeniden hizalama atlandi, "
                        "arkasindan deneme gelmeyecek", attempt, HOOK_PICKUP_ATTEMPTS)
            return
        try:
            await on_retry(attempt)
        except asyncio.CancelledError:
            raise
        except Exception:  # noqa: BLE001 -- hizalama denemeyi dusuremez
